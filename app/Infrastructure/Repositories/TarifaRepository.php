    ...
    public function consultarTarifas(?string $claveZona): array
    {
        ...
        return Cache::remember($cacheKey, 300, function () use ($claveZona) {
            try {
                ...
                if ($claveZona) {
                    $resultados = DB::connection('maniobras')->select('SET ANSI_NULLS ON; SET ANSI_WARNINGS ON; EXEC proc_pdm_consultar_tarifas ?', [$claveZona]);
                } else {
                    $resultados = DB::connection('maniobras')->select('SET ANSI_NULLS ON; SET ANSI_WARNINGS ON; EXEC proc_pdm_consultar_tarifas');
                }
