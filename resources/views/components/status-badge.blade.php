...
@php
    ...
    $classes = match($normalizado) {
        'activo', 'a'          => 'bg-emerald-500/15 text-emerald-800 dark:text-emerald-300 border-emerald-500/30 shadow-sm shadow-emerald-500/10',
        'inactivo', 'i'        => 'bg-rose-500/15 text-rose-800 dark:text-rose-300 border-rose-500/30 shadow-sm shadow-rose-500/10',
        'en proceso'           => 'bg-amber-500/15 text-amber-800 dark:text-amber-300 border-amber-500/30 shadow-sm shadow-amber-500/10',
...
<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border transition-colors duration-150 {{ $classes }}">
    @if($normalizado === 'activo' || $normalizado === 'a')
